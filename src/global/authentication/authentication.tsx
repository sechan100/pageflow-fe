 ...
 */
const [ Provider, useStore ] = createStoreContext<boolean, UseAuthentication>((isAuthed, set) => ({
...
export const UseAuthenticationProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    setAuth(isSessionFlagValid());
  }, [])
  
  return (
    <Provider data={auth} onDataChange={(s, isAuthenticated) => s.setState({ isAuthenticated })}>
      {children}
    </Provider>
  )
}
